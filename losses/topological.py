...
def topological_loss(y_true, y_pred):
    """
    Computes the topological loss between y_true and y_pred as in the paper.
    Note: This should probably not be used alone, but in combination with a
    'usual' loss fuct.
    :param y_true:
    :param y_pred:
    :return: a scalar tensor
    """
    pd_lh, bcp_lh, dcp_lh = compute_persistence_2DImg_1DHom(y_pred)
    pd_gt, bcp_gt, dcp_gt = compute_persistence_2DImg_1DHom(y_true)
    return tf.constant(compute_topological_loss(pd_lh, pd_gt))


def get_total_loss(Lambda=0.5):
    """
    Returns a function that calculates Lambda * topological loss + (1-Lambda) * binary crossentropy
    :param Lambda: should be in [0,1] for it to make sense
    :return: a loss function that is a combination of topo and crossentr loss
    """
    def _loss(y_true, y_pred):
        return (1 - Lambda) * tf.losses.binary_crossentropy(y_true, y_pred)
        + Lambda * topological_loss(y_true, y_pred)
